 ...
 *
 * @author li.liangzhe
 * @create 2017-12-25 18:02
 **/
package com.anzlee.generalapi.coder.compiler;
                ...
                File[] childrenFiles = sourceFile.listFiles(new FileFilter() {
                    public boolean accept(File pathname) {
                        if (pathname.isDirectory()) {
                            try {
                                new CopyDirectoryUtils().copyDirectiory(pathname.getPath(), targetDir + pathname.getPath().substring(pathname.getPath().indexOf("src") + 3, pathname.getPath().length()));
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                            return true;
                            ...
                            try {
                                new CopyDirectoryUtils().copyFile(pathname, new File(targetDir + pathname.getPath().substring(pathname.getPath().indexOf("src") + 3, pathname.getPath().length())));
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                            return false;
